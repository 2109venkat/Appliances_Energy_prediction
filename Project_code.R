...
X_train <- read.csv("/Users/asmitha/Desktop/train_x.csv")
View(X_train)
X_train <- X_train[, 2:22]
View(X_train)

X_test <- read.csv("/Users/asmitha/Desktop/test_x.csv")
View(X_test)
X_test <- X_test[, 2:22]
View(X_test)

#splitting our dependent variable into Y_train and Y_test
Y_train <- read.csv("/Users/asmitha/Desktop/train_y.csv", header= FALSE)
Y_train <- Y_train[, 2]
View(Y_train)

Y_test <- read.csv("/Users/asmitha/Desktop/test_y.csv", header= FALSE)
Y_test<- Y_test[, 2]
View(Y_test)
...
set.seed(100)
marsModel = train(x=X_train, y=Y_train, method="earth", preProc=c("center", "scale"), 
...
library(pls)
set.seed(567)
pls <- train(X_train, Y_train, method="pls",tuneLength=10,  preProcess= p_pro, trControl= ctrl)
pls$results
